...
async def main():
    async with aiohttp.ClientSession() as session:
        final_dict = {}
        for action, urls in TENOR_URLS.items():
            tasks = [get_direct_url(session, url) for url in urls]
            results = await asyncio.gather(*tasks)
            final_dict[action] = [r for r in results if r != "ERROR"]
        
        print("FINAL_GIFS_DICT = {")
